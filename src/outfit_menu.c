...
static const u8 sFontColors[][3] = { // bgColor, textColor, shadowColor
    ...
    [COLORID_HEADER] = {6,                      TEXT_COLOR_WHITE,      10},
};
...
// Dedicated palette for WIN_HEADER so the {A_BUTTON}/{B_BUTTON}/{DPAD_NONE} keypad icons
// (which are blitted using whatever's loaded at indices 1-3) get proper contrast,
// instead of inheriting main.gbapal's colors at those slots.
static const u16 sHeaderPalette[16] = {
    [0]  = RGB_BLACK,
    [1]  = RGB(31, 31, 31), // white - icon circle / normal text
    [2]  = RGB(4, 4, 6),    // near-black - icon letter, for contrast against index1
    [3]  = RGB(22, 22, 22), // light gray - icon bevel highlight
    [6]  = RGB(31, 11, 11), // red - header background fill
    [10] = RGB(10, 9, 9),   // dark gray - text shadow
};
...
static bool32 SetupOutfitMenu_Graphics(void)
{
    ...
    case 1:
        ...
        {
            DecompressDataWithHeaderWram((const u32 *)sTilemap, sOutfitMenu->tilemapBuffers[0]);
            DecompressDataWithHeaderWram((const u32 *)sScrollingBG_Tilemap, sOutfitMenu->tilemapBuffers[1]);
            sOutfitMenu->gfxState++;
        }
        break;
    case 2:
...
static void SetupOutfitMenu_PrintStr(void)
{
    ...
    FillWindowPixelBuffer(WIN_HEADER, PIXEL_FILL(6));
    PrintTexts(WIN_HEADER, FONT_NORMAL, 8, 1, COLORID_HEADER, sText_OutfitMenuTitle);
    PrintTexts(WIN_HEADER, FONT_NORMAL, 72, 1, COLORID_HEADER, sText_OutfitMenuControls);
    CopyWindowToVram(WIN_HEADER, COPYWIN_FULL);
}

static inline void SetupOutfitMenu_Sprites_DrawTrainerSprite(bool32 update, bool32 unlocked)
{
    u32 trainerPicId = GetPlayerTrainerPicIdByOutfit(sOutfitMenu->idx, gSaveBlock2Ptr->playerGender);
    u32 frontPalSlot = 9;
    u32 backPalSlot = 12;
    ...
    sOutfitMenu->spriteIds[GFX_FTS] = CreateTrainerPicSprite(trainerPicId, TRUE, 32+27, 32+32, frontPalSlot, TAG_NONE);
    sOutfitMenu->spriteIds[GFX_BTS] = CreateTrainerPicSprite(trainerPicId, FALSE, 32+117, 32+32, backPalSlot, TAG_NONE);
    if (!unlocked)
    {
        // bc we're directly tinting idx 1-15, skipping idx 0
...
static void SpriteCB_Overworld(struct Sprite *s)
{
    u32 idx = s->data[0];
    u32 i = sOutfitMenu->list[sOutfitMenu->grid->topLeftItemIndex + idx];
    // play anim only if it's currently picked AND that it's unlocked
    if (idx == sOutfitMenu->grid->selectedItem && GetOutfitStatus(i))
    {
...
static void SetupOutfitMenu_Grids(void)
{
    ...
    for (i = 0; i < GRID_ROWS; i++)
        sOutfitMenu->currentOutfitSpriteIds[i] = SPRITE_NONE;
...
static void FreeOutfitMenuResources(void)
{
    ...
    {
        if (sOutfitMenu->currentOutfitSpriteIds[i] != SPRITE_NONE)
...
u16 GetPlayerTrainerPicIdByOutfit(u16 outfitId, u8 gender)
{
    if (outfitId > OUTFIT_NONE && outfitId < OUTFIT_COUNT)
        ...
        return gOutfits[DEFAULT_OUTFIT].trainerPics[gender];
}
